...
Find out what you can do with a fully managed
...
always free with your account. **[Start free >](https://aka.ms/k8sur)**


Then, try these labs to master the basic and
advanced tasks required to deploy a multicontainer application to Kubernetes on Azure
Kubernetes Service (AKS). **[Try now >](https://aka.ms/k8surworkshops)**
...
_**Dive into the Future of Infrastructure**_


_**Brendan Burns, Joe Beda, and Kelsey Hightower**_
...
_For my Dad, who helped me fall in love with computers by bringing home punch cards_
...
This Dockerfile produces two images. The first is the _build_ image, which contains the
...
_Figure 5-1. An example Pod with two containers and a shared filesystem_
...
Kubernetes groups multiple containers into a single atomic unit called a _Pod_ . (The
...
tion of the Kubernetes API object. Kubernetes strongly believes in _declarative configu‐_
_ration_ . Declarative configuration means that you write down the desired state of the
...
Declarative configuration is different from _imperative configura‐_
_tion_, where you simply take a series of actions (e.g., `apt-get`
...
_kuard-pod.yaml_ and then using `kubectl` commands to load that manifest to
Kubernetes.


_Example 5-1. kuard-pod.yaml_
...
When a Pod is deleted, it is _not_ immediately killed. Instead, if you run `kubectl get`
`pods` you will see that the Pod is in the `Terminating` state. All Pods have a termina‐
tion _grace period_ . By default, this is 30 seconds. When a Pod is transitioned to
...
tiple instances of a Pod, you need to use `PersistentVolume` s, described at the end of
this chapter.
...
this case the `kuard` web interface) at _[http://localhost:8080](http://localhost:8080)_ .
...
ize `tcpdump` packet captures. Suppose you had a file called _/captures/capture3.txt_
...
Let’s say you want to copy _$HOME/config.txt_ to a remote container. In this case, you
...
alive for you using a _process health check_ . This health check simply ensures that the
...
_Example 5-2. kuard-pod-health.yaml_
...
Point your browser to _[http://localhost:8080](http://localhost:8080)_ . Click the “Liveness Probe” tab. You
...
makes a distinction between _liveness_ and _readiness_ . Liveness determines if an applica‐
...
Generally speaking, we measure this efficiency with the _utilization_ metric. Utilization
...
Kubernetes allows users to specify two different resource metrics. Resource _requests_
specify the minimum amount of a resource required to run the application. Resource
_limits_ specify the maximum amount of a resource that an application can consume.
...
_Example 5-3. kuard-pod-resreq.yaml_
...
_Example 5-4. kuard-pod-reslim.yaml_
...
_Example 5-5. kuard-pod-vol.yaml_
...
The previous example uses the `hostPath` volume type. The volume created is _/var/lib/_
_kuard_ on the host.
...
_Example 5-6. kuard-pod-full.yaml_
...
concepts. Labels and annotations let you work in sets of things that map to how _you_
...
_Labels_ are key/value pairs that can be attached to Kubernetes objects such as Pods and
...
_Annotations_, on the other hand, provide a storage mechanism that resembles labels:
...
book _[Site Reliability Engineering](http://shop.oreilly.com/product/0636920041528.do)_ by Betsy Beyer et al. (O’Reilly) for
...
_Figure 6-1. Visualization of labels applied to our deployments_
...
ning in the cluster. We should have a total of six `kuard` Pods across our three environ‐
...
You may see a new label that we haven’t seen yet: `pod-template-`
`hash` . This label is applied by the deployment so it can keep track of
...
_Table 6-2. Selector operators_
...
The older form of specifying selectors (used in `ReplicationController` s and serv‐
...
to associate it with, consider storing that data in some other, more appropriate data‐
base.
...
problems when it comes to _finding_ those things. Most of the traditional network
...
The general name for this class of problems and solutions is _service discovery_ . Servicediscovery tools help solve the problem of finding which processes are listening at
...
Furthermore, that service is assigned a new type of virtual IP called a _cluster IP_ . This
...
forward working by accessing the `alpaca` Pod at _[http://localhost:48858](http://localhost:48858)_ :
...
( `alpaca-prod` ). You can also refer to a service in another namespace with `alpaca-`
`prod.default` . And, of course, you can use the fully qualified service name ( `alpaca-`
`prod.default.svc.cluster.local.` ). Try each of these out in the “DNS Query”
section of `kuard` .
...
Point your browser to _[http://localhost:48858](http://localhost:48858)_ and you should see the debug page for
...
Press Ctrl-C to exit out of both the `port-forward` and `watch` commands in your ter‐
minals.
...
Now if you point your browser to _http://localhost:8080_ you will be connected to that
...
Here we see that we have an address of 104.196.248.204 now assigned to the `alpaca-`
`prod` service. Open up your browser and try!
...
Now open up _another_ terminal window and delete and recreate the deployment back‐
ing `alpaca-prod` :
...
_Figure 7-1. Configuring and using a cluster IP_
...
Now point your browser to _http://localhost:48858_ to see the status page for this server.
...
_Table 7-1. Service environment variables_
...
that make up a larger application. In addition, using _just_ environment variables seems
...
Kubernetes calls its HTTP-based load-balancing system _Ingress_ . Ingress is a
...
1 [The Open Systems Interconnection (OSI) model is a standard way to describe how different networking lay‐](https://en.wikipedia.org/wiki/OSI_model)
...
_Figure 8-1. The typical software Ingress controller configuration_
...
The Contour project is hosted at _[https://github.com/heptio/contour](https://github.com/heptio/contour)_ .
It was created by Heptio [2] in collaboration with real-world custom‐
...
This one line works for most configurations. It creates a namespace called `heptio-`
`contour` . Inside of that namespace it creates a deployment (with two replicas) and an
...
can set up a local configuration by editing your _/etc/hosts_ file to add an IP address.
...
on Windows the file is usually at _C:\Windows\System32\drivers\etc\hosts_, and for
...
For `<` _`ip-address`_ `>`, fill in the external IP address for Contour. If all you have is a host‐
name (like from AWS), you can get an IP address (that may change in the future) by
executing `host -t a` _`<address>`_ .
...
_Example 8-1. simple-ingress.yaml_
...
This sets things up so that _any_ HTTP request that hits the Ingress controller is for‐
...
direct traffic based on that header. Let’s add _another_ Ingress object for directing traffic
to the `alpaca` service for any traffic directed to `alpaca.example.com` (see
Example 8-2).


_Example 8-2. host-ingress.yaml_
...
Regardless, you should now be able to address the `alpaca` service via _http://_
_alpaca.example.com_ . If instead you reach the service endpoint via other methods, you
...
_http://bandicoot.example.com_ to the `bandicoot` service, but we also send _http://bandi‐_
_coot.example.com/a_ to the `alpaca` service. This type of scenario can be used to host
...
_Example 8-3. path-ingress.yaml_
...
is an “internal” link for the app (in which case it should instead be `/subpath/app-`
`path` ) or a link to some other app. For this reason, it is probably best to avoid sub‐
...
`key <` _`private-key-pem-file`_ `>` .


_Example 8-4. tls-secret.yaml_
...
_Example 8-5. tls-ingress.yaml_
...
often come at a significant cost. To help solve this problem, there is a [non-profit](https://letsencrypt.org/)
[called “Let’s Encrypt” running a free Certificate Authority that is API-driven. Since it](https://letsencrypt.org/)
...
very simple to use. The missing piece is an open source project called `[cert-manager](https://github.com/jetstack/cert-manager)`
...
The most popular generic Ingress controller is probably the open source [NGINX](https://github.com/kubernetes/ingress-nginx/)
[ingress controller. Be aware that there is also a commercial controller based on the](https://github.com/kubernetes/ingress-nginx/)
...
[tures and options exposed via annotations.](http://bit.ly/2LMRi7N)


[Ambassador and Gloo are two other Envoy-based Ingress controllers that are focused](https://github.com/datawire/ambassador)
on being API gateways.


[Traefik is a reverse proxy implemented in Go that also can function as an Ingress](https://traefik.io/)
...
[This is an active area of innovation in the Kubernetes community through the Net‐](https://github.com/kubernetes/community/tree/master/sig-network)
[work Special Interest Group.](https://github.com/kubernetes/community/tree/master/sig-network)
...
example of a _reconciliation loop_ . Such loops are fundamental to most of the design
...
The central concept behind a reconciliation loop is the notion of _desired_ state versus
_observed_ or _current_ state. Desired state is the state you want. With a ReplicaSet, it is
...
and ReplicaSets enables several important behaviors, discussed in the following sec‐
tions.
...
edit the _kuard-rs.yaml_ configuration file and set the `replicas` count to `3` :
...
all of these scenarios via _Horizontal Pod Autoscaling_ (HPA).
...
not simply called “autoscaling.” Kubernetes makes a distinction between _horizontal_
scaling, which involves creating additional replicas of a Pod, and _vertical_ scaling,
...
Kubernetes, but it is planned. Additionally, many solutions also enable _cluster_
...
requests. This original demo was based on the `kubectl rolling-`
`update` command, which is still available in the command-line
...
Save this YAML file as _kuard-deployment.yaml_, then you can create it using:
...
listing for brevity. You also need to run `kubectl replace --save-`
`config` . This adds an annotation so that, when applying changes in
...
field in the Deployment object. Also, do not update the `change-`
`cause` annotation when doing simple scaling operations. A modifi‐
...
ply a rollout in reverse (e.g., from _v2_ to _v1_, instead of from _v1_ to _v2_ ), and all of the
...
this way, `kubectl rollout undo` is equivalent to `kubectl rollout undo --to-`
...
_Figure 10-1. Diagrams of both decoupled (left) and coupled (right) application_
_architectures_
...
_have_ to specify readiness health checks for the containers in your
...
onds _after_ seeing a Pod become healthy before moving on to updating the next Pod.
...
It is important to note that this timeout is given in terms of deployment _progress_, not
...
_Figure 10-2. The Kubernetes deployment lifecycle_
...
new virtual machines. This dynamic _immutable infrastructure_ approach can cause
...
text of ReplicaSets (e.g., `kubectl logs <` _`pod-name`_ `>` ) is equally
...
_Example 11-2. nginx-fast-storage.yaml_
...
**Limiting DaemonSets to Specific Nodes** **|** **135**
...
times until reaching a fixed completion
count
...
_Example 12-1. job-oneshot.yaml_
...
off this automatic behavior and manually specify labels and selec‐
...
_Example 12-2. job-oneshot-failure1.yaml_
...
If we let this run for a bit and then look at related Pods we’ll find something interest‐
ing:
...
_Example 12-3. job-parallel.yaml_
...
You can open your browser to _[http://localhost:8080](http://localhost:8080)_ and see the `kuard` interface.
...
_Example 12-5. service-queue.yaml_
...
of work items. `curl` will communicate to the work queue through the `kubectl port-`
`forward` we set up earlier, as shown in Example 12-6.


_Example 12-6. load-queue.sh_
...
_Example 12-7. job-consumers.yaml_
...
You can save this file as _cron-job.yaml_, and create the CronJob with `kubectl create`
`-f cron-job.yaml` . If you are interested in the current state of a CronJob, you can use
`kubectl describe` _`<cron-job>`_ to get the details.
...
First, suppose we have a file on disk (called _my-config.txt_ ) that we want to make avail‐
able to the Pod in question, as shown in Example 13-1.


_Example 13-1. my-config.txt_
...
The interesting stuff happens when you try to _use_ a ConfigMap.
...
**154** **|** **Chapter 13: ConfigMaps and Secrets**
...
_Example 13-2. kuard-config.yaml_
...
the correct substitution with a special `$(` _`<env-var-name>`_ `)` syntax.
...
Now point your browser at _[http://localhost:8080](http://localhost:8080)_ . We can look at how we’ve injected
...
_Figure 13-1. kuard showing its environment_


**156** **|** **Chapter 13: ConfigMaps and Secrets**
...
(prepended with _.._ ) that are used to do a clean swap of new values when the Config‐
Map is updated.


_Figure 13-2. The /config directory as seen through kuard_
...
environments and distributable through public Docker reposito‐
ries.
...
**158** **|** **Chapter 13: ConfigMaps and Secrets**
...
With the _kuard.crt_ and _kuard.key_ files stored locally, we are ready to create a secret.
...
_kuard.crt_ and _kuard.key_ . Mounting the `kuard-tls` secrets volume to `/tls` results in
the following files:
...
_Example 13-3. kuard-secret.yaml_
...
Now navigate your browser to _[https://localhost:8443](https://localhost:8443)_ . You should see some invalid cer‐
tificate warnings as this is a self-signed certificate for _kuard.example.com_ . If you navi‐
...
**160** **|** **Chapter 13: ConfigMaps and Secrets**


_Image pull secrets_ leverage the secrets API to automate the distribution of private reg‐
...
_Example 13-4. kuard-secret-ips.yaml_
...
_Table 13-1. ConfigMap and secret key examples_
...
**162** **|** **Chapter 13: ConfigMaps and Secrets**
...
**Managing ConfigMaps and Secrets** **|** **163**
...
push a new version with `kubectl replace -f <` _`filename`_ `>` . You can also use `kubectl`
`apply -f` _`<filename>`_ if you previously created the resource with `kubectl apply` .
...
that it _would have_ sent to the API server to `stdout` . We then pipe that to `kubectl`
...
**164** **|** **Chapter 13: ConfigMaps and Secrets**
...
Every request to Kubernetes is first _authenticated_ . Authentication provides the iden‐
...
A _role_ is a set of abstract capabilities. For example, the `appdev` role might represent
the ability to create Pods and services. A _role binding_ is an assignment of a role to one
...
_Table 14-1. Common Kubernetes RBAC verbs_
...
default `ClusterRole` s that are defined in the code of the API server itself. This means
...
supports the usage of an _aggregation rule_ to combine multiple roles together in a new
...
Like with all other aggregations or groupings in Kubernetes, the `ClusterRole` s to be
...
tion, remember that all Kubernetes objects are deployed into _namespaces_ . Imagine
...
_Example 15-1. dns-service.yaml_
...
_Example 15-2. external-ip-service.yaml_
...
_Example 15-3. external-ip-endpoints.yaml_
...
_Example 15-4. nfs-volume.yaml_
...
_Example 15-5. nfs-volume-claim.yaml_
...
_Example 15-6. mysql-replicaset.yaml_
...
_Example 15-7. mysql-service.yaml_
...
Many clusters also include _dynamic volume provisioning_ . With dynamic volume pro‐
...
_Example 15-8. storageclass.yaml_
...
_Example 15-9. dynamic-volume-claim.yaml_
...
_Example 15-10. mongo-simple.yaml_
...
_Example 15-11. mongo-service.yaml_
...
you’ll need to change it in the _mongo.yaml_ StatefulSet definition as
well.
...
_Example 15-12. mongo-configmap.yaml_
...
For persistent storage, you need to mount a persistent volume into the _/data/db_ direc‐
...
volume claim. Instead, you need to add a _persistent volume claim template_ . You can
...
_Figure 16-1. API server request flow_
...
custom resources, the name is special. It has to be the following format: _`<resource-`_
_`plural>`_ `.` _`<api-group>`_ . The reason for this is to ensure that each resource definition
...
Now use _loadtest-resource.yaml_ to create this resource:
...
You can now use this _loadtest.yaml_ file to create a resource just like you would with
any built-in type:
...
_Figure 16-2. CustomResourceDefinition interactions_
...
`[client-go](https://godoc.org/k8s.io/client-go/informers)` library.
...
scope of this book, but there are lots of resources online, including the [complete](http://bit.ly/2Jn89we)
[Kubernetes API specification.](http://bit.ly/2Jn89we)
...
LoadTest object has a valid scheme (e.g., _http_ or _https_ ) or that `requestsPerSecond` is a
...
and it will generate two files, _server.csr_ and _server-key.pem_ .
...
changing the `kind` field in the YAML object and saving the file as _mutating-_
_controller.yaml_ . Then create the controller by running:
...
application. It is important to note that you should _not_ use the Kubernetes API server
...
[experience. Fortunately, there is a great deal of code to help you out. The Kubebuilder](https://kubebuilder.io/)
[project contains a library of code intended help you easily build reliable Kubernetes](https://kubebuilder.io/)
...
[The Jupyter Project is a web-based interactive scientific notebook for exploration and](https://jupyter.org)
...
Create a file named _jupyter.yaml_ with these contents. Once you have created this file,
...
Finally, you can visit _http://localhost:8888/?token=<token>_, inserting the token that
...
[tutorials to get oriented to Jupyter if you are so inclined on the Jupyter project site.](http://jupyter.org/try)


**Parse**


[The Parse server is a cloud API dedicated to providing easy-to-use storage for mobile](https://parse.com)
...
you can get one for free at _[https://docker.com](https://docker.com)_ .
...
_Example 17-2. parse-service.yaml_
...
secure the connection with HTTPS. You can see the `[parse-server](https://github.com/parse-community/parse-server)` GitHub page for
...
[Ghost is a popular blogging engine with a clean interface written in JavaScript. It can](https://ghost.org/)
...
_Example 17-3. ghost-config.js_
...
Once you have this configuration file saved to _ghost-config.js_, you can create a Kuber‐
...
One thing to note here is that we are copying the _config.js_ file from a different loca‐
...
ConfigMap into _/var/lib/ghost_ .
...
Then visit _[http://localhost:8001/api/v1/namespaces/default/services/ghost/proxy/](http://localhost:8001/api/v1/namespaces/default/services/ghost/proxy/)_ in
...
ure the master, create a file named _master.conf_ that contains the code in
Example 17-5.


_Example 17-5. master.conf_
...
port) and store its files in the _/redis-data_ directory.


The slave configuration is identical, but it adds a single `slaveof` directive. Create a
file named _slave.conf_ that contains what’s in Example 17-6.


_Example 17-6. slave.conf_
...
We also need a configuration for the Redis sentinel. Create a file named _sentinel.conf_
with the contents of Example 17-7.


_Example 17-7. sentinel.conf_
...
ate a file named _init.sh_ containing the code in Example 17-8.


_Example 17-8. init.sh_
...
_sentinel.sh_ containing the code in Example 17-9.


_Example 17-9. sentinel.sh_
...
_Example 17-10. redis-service.yaml_
...
You can see that there are two containers in this Pod. One runs the _init.sh_ script that
...
component or layer (e.g., _frontend_, _batch work queue_, etc.). Though early on this
...
release branch ( `git cherry-pick` _`<edit>`_ ), and finally, you tag this branch with the
...
sions or tags. When adding a new configuration, it is done to the files in the _current_
directory.


When creating a new release, the _current_ directory is copied to create a new directory
...
use parameterized environments. Parameterized environments use _templates_ for the
bulk of their configuration, but they mix in a limited set of _parameters_ to produce the
...
In general they all divide the files into a _template_ file, which contains the bulk of the
configuration, and a _parameters_ file, which can be combined with the template to
...
The following gives examples of how to parameterize configurations using [Helm, a](https://helm.sh)
...
To pass a parameter for this value you use a _values.yaml_ file with contents like:
...
known, and that is why you want to understand the _average_ time. Managing software
...
Visit the [Hypriot downloads page and download the latest stable image. Unzip the](http://blog.hypriot.com/downloads/)
image, and you should now have an _.img_ file. The Hypriot project also provides really
...
the internet less safe for everyone. Please change your default pass‐
words!
...
world. Edit the _/boot/user-data_ file. Update the WiFi SSID and password to match
...
network. To do this, edit _/etc/network/interfaces.d/eth0_ to read:
...
Then configure the DHCP server as follows ( _/etc/dhcp/dhcpd.conf_ ):
...
You may also need to edit _/etc/defaults/isc-dhcp-server_ to set the `INTERFACES` environ‐
...
Remember to edit the _/etc/hostname_ file to rename this machine to `node-1` .
...
Edit _/etc/sysctl.conf_ and set `net.ipv4.ip_forward=1` to turn on IP forwarding. You
...
choose to do this you will still want to edit _/etc/sysctl.conf_ to make the setting perma‐

nent.


Then edit _/etc/rc.local_ (or the equivalent) and add `iptables` rules for forwarding
...
10.0.0.4). Edit the _/etc/hostname_ file on each machine to name them `node-2` and
`node-3`, respectively.
...
Validate this by first looking at _/var/lib/dhcp/dhcpd.leases_, and then SSH to the nodes
...
The first is to edit _/etc/hosts_ on each machine to map the names to the right addresses.
On each machine, add:
...
copy the _$HOME/.ssh/id_rsa.pub_ file into `/home/pirate/.ssh/authorized_keys` on
...
[The easiest way to manage this is to use the Flannel tool created by CoreOS. Flannel](http://bit.ly/2vgBsKU)
supports a number of different routing modes; we will use the `host-gw` mode. You
[can download an example configuration from the Flannel project page:](https://github.com/coreos/flannel)
...
Once you have your updated _kube-flannel.yaml_ file, you can create the Flannel net‐
working setup with:
...
To access this UI, you can run `kubectl proxy` and then point your browser to _[http://](http://localhost:8001/ui)_
_[localhost:8001/ui](http://localhost:8001/ui)_, where _localhost_ is local to the master node in your cluster. To view
this from your laptop/desktop, you may need to set up an SSH tunnel to the root
node using `ssh -L8001:localhost:8001` _`<master-ip-address>`_ .
...
The animal on the cover of _Kubernetes: Up and Running_ is an Atlantic white-sided
dolphin ( _Lagenorhynchus acutus_ ). As its name suggests, the white-sided dolphin has
...
from _British Quadrupeds_ . The cover fonts are Gilroy Semibold and Guardian Sans.
